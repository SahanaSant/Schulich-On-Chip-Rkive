# Timing-closure candidate constraints for the complete two-clock UART macro.
# Both domains target 50 MHz and are asynchronous to one another.
#
# Macro interface contract:
# - Inputs may arrive up to 2 ns after their associated clock edge.
# - Outputs must settle at least 2 ns before the next associated clock edge.
# This reserves 20% of the 20 ns cycle for logic outside the UART macro.
# The final MCU-level timing run must validate this provisional contract.
...
set_input_delay 2.0 -clock periph_clk [get_ports uart_rx_i]
set_output_delay 2.0 -clock periph_clk [get_ports uart_tx_o]
